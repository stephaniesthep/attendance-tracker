...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const attendanceRecords: AttendanceRecord[] = await Promise.all(
    attendances.map(async (attendance) => {
      ...
      return {
        ...
        userDivision: attendance.user.department || "No Division",
  ...
  const offDayRecords: AttendanceRecord[] = await Promise.all(
    offDayUsers.map(async (user) => {
      ...
      return {
        id: `off-day-${user.id}-${dateString}`, // Create a unique ID for off day records
        userName: user.name,
  ...
  return { records: allRecords, currentDate: dateString };
}
